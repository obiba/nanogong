...
public class ImaADPCMData extends BlockAudioData {
     ...
     * @throws gong.audio.AudioDataException invalid data/request
     */
    public void receiveHeaderFromStream(InputStream stream, OutputStream fout) throws IOException, AudioDataException {
        DataInputStream dataStream = new DataInputStream(stream);
        byte[] buffer = new byte[4];
        
        receiveByteArrayFromStream(dataStream, buffer, 0, 4);               // Chunk Id
        if (!(new String(buffer, 0, 4).equals("RIFF"))) throw new AudioDataException("Invalid chunk id ('RIFF').");
        int riffSize = swapInt(dataStream.readInt());               // Chunk size
        receiveByteArrayFromStream(dataStream, buffer, 0, 4);               // Riff type
        if (!(new String(buffer, 0, 4).equals("WAVE"))) throw new AudioDataException("Invalid riff type ('WAVE').");
        receiveByteArrayFromStream(dataStream, buffer, 0, 4);                   // Chunk ID
        if (!(new String(buffer, 0, 4).equals("fmt "))) throw new AudioDataException("Invalid chunk id ('fmt ').");
        int chunkSize = swapInt(dataStream.readInt());              // Chunk Size
        if (chunkSize != 20) throw new AudioDataException("Invalid chunk size.");
        short code = swapShort(dataStream.readShort());             // Compression Code
        if (code != 17) throw new AudioDataException("Invalid compression code.");
        short channel = swapShort(dataStream.readShort());          // Channel
        if (channel != 1) throw new AudioDataException("Invalid channel.");
        ...
        if (sampleSize != 4) throw new AudioDataException("Invalid sample size.");
        short extraBytes = swapShort(dataStream.readShort());       // Extra Bytes
        if (extraBytes != 2) throw new AudioDataException("Invalid extra bytes.");
        short samplesPerBlock = swapShort(dataStream.readShort());  // Samples per Block
        this.samplesPerBlock = (blockAlign - 4) * 2 + 1;
        if (this.samplesPerBlock != samplesPerBlock) throw new AudioDataException("Invalid samples per block.");
     ...
     * @throws java.io.IOException failed to receive block data from stream
     * @throws gong.audio.AudioDataException invalid data/request
     */
    protected void receiveDataFromStream(InputStream stream, OutputStream fout, boolean synchronous) throws IOException, AudioDataException {
        DataInputStream dataStream = new DataInputStream(stream);
        byte[] buffer = new byte[4];
        int chunkSize;
        ...
        if (chunkSize % blockAlign != 0) throw new AudioDataException("Invalid block align.");
        ...
        if (synchronous) {
            ...
            if (chunkSize != blockData.size() * blockAlign) throw new AudioDataException("Invalid block data.");
        } else {
    ...
    protected class ImaADPCMTransferThread extends TransferThread {
        ...
        public void run() {
                ...
                while (inProgress) {
                    ...
                    if (listener != null) {
                        ...
                        if (lastUpdatedTime == 0 || lastUpdatedTime + UPDATE_INTERVAL < time) {
                            listener.received(ImaADPCMData.this, getAvailable());
            ...
            if (listener != null) listener.finish(ImaADPCMData.this, getAvailable());
    ...
    protected class ImaADPCMBlock extends Block {
         ...
         * @return the sample
         * @throws java.io.IOException failed to read a sample
         * @throws gong.audio.AudioDataException invalid data/request
         */
         ...
         * @throws java.io.IOException failed to write the sample
         * @throws gong.audio.AudioDataException invalid data/request
         */
        public synchronized void write(int sample) throws IOException, AudioDataException {
         ...
         * @throws java.io.IOException failed to seek to the position
         * @throws gong.audio.AudioDataException invalid data/request
         */
        public synchronized int seek(int position) throws IOException, AudioDataException {
            if (position < 0 || position >= size) throw new AudioDataException("Invalid seek position.");
         ...
         * @param stream the output stream
         * @throws java.io.IOException failed to send the block
         * @throws gong.audio.AudioDataException invalid data/request
         */
        public synchronized void sendToStream(OutputStream stream) throws IOException, AudioDataException {
         ...
         * @throws java.io.IOException failed to receive the block data
         */
        public synchronized void receiveFromStream(InputStream stream, OutputStream fout) throws IOException {
            DataInputStream dataStream = new DataInputStream(stream);
